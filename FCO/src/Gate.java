...
public class Gate {
    ...
    public void setDefaultBounds() {
        if (type == 7 || type == 9) {
            addExit(new Rectangle2D.Double((int) (gateBounds.getX() + gateBounds.getWidth() / 2) - 5, (int) gateBounds.getY() - 5,10 ,10));
        } else if (type == 6 || type == 8 || type == 10) {
            addEntry(new Rectangle2D.Double(gateBounds.getX() - 5, gateBounds.getY() + (int) (gateBounds.getHeight() / 2) - 5,10 ,10));
            addExit(new Rectangle2D.Double(gateBounds.getX() + gateBounds.getWidth() - 5, gateBounds.getY() + (int) (gateBounds.getHeight() / 2) - 5,10 ,10));
        } else {
            addEntry(new Rectangle2D.Double(gateBounds.getX() - 5, gateBounds.getY() + (1 * gateBounds.getHeight() / 3) - 5,10 ,10));
            addEntry(new Rectangle2D.Double(gateBounds.getX() - 5, gateBounds.getY() + (2 * gateBounds.getHeight() / 3) - 5,10 ,10));
            addExit(new Rectangle2D.Double(gateBounds.getX() + gateBounds.getWidth() - 5, gateBounds.getY() + (int) (gateBounds.getHeight() / 2) - 5,10 ,10));
        }
    }
}
